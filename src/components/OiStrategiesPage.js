...
function Metric({ label, value, sub, tone }) {
  return <div className="card" style={{ padding: 14, borderTop: `3px solid ${tone || 'var(--border-light)'}` }}><div style={{ color: 'var(--text-secondary)', fontSize: 11 }}>{label}</div><div style={{ fontSize: 20, fontWeight: 700, marginTop: 5 }}>{value}</div>{sub && <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 4 }}>{sub}</div>}</div>;
}
...
export default function OiStrategiesPage() {
  ...
  return <section className="section" style={{ marginTop: 4 }}>
    ...
    <div className="grid grid-4" style={{ marginTop: 14 }}><Metric label={`MAX PROFIT (${lots} LOT${Number(lots) === 1 ? '' : 'S'})`} value={money(preview?.metrics?.max_profit) || 'Waiting for OI data'} sub={preview ? `${money(preview.metrics.max_profit_per_lot)} per lot` : null} tone="var(--accent-green)" /><Metric label={`MAX LOSS (${lots} LOT${Number(lots) === 1 ? '' : 'S'})`} value={money(preview?.metrics?.max_loss) || 'Waiting for OI data'} sub={preview ? `${money(preview.metrics.max_loss_per_lot)} per lot` : null} tone="var(--accent-red)" /><Metric label={`MARGIN (${lots} LOT${Number(lots) === 1 ? '' : 'S'})`} value={money(preview?.metrics?.broker_margin_required || preview?.metrics?.margin_required) || 'Waiting for OI data'} sub={preview ? `${money(preview.metrics.broker_margin_required_per_lot || preview.metrics.margin_required_per_lot)} per lot` : null} tone="var(--accent-yellow)" /><Metric label="BREAKEVEN" value={preview?.metrics?.breakevens?.map(price).join(' / ') || 'Waiting for OI data'} tone="var(--accent-blue)" /></div>
