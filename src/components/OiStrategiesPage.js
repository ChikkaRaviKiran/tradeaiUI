...
export default function OiStrategiesPage() {
  ...
  const buildPreview = async () => {
    if (!market) return;
    try { setBusy(true); setError(''); const result = await previewOiStrategy({ strategy, symbol: market.symbol, lots: Number(lots) }); setPreview(result.data); }
  ...
  return <section className="section" style={{ marginTop: 4 }}>
    ...
    <div className="grid grid-4" style={{ marginTop: 14 }}><Metric label="MAX PROFIT / LOT" value={preview ? money(preview.metrics.max_profit_per_lot) : 'Waiting for OI data'} tone="var(--accent-green)" /><Metric label="MAX LOSS / LOT" value={preview ? money(preview.metrics.max_loss_per_lot) : 'Waiting for OI data'} tone="var(--accent-red)" /><Metric label="MARGIN / LOT (EST.)" value={preview ? money(preview.metrics.margin_required_per_lot) : 'Waiting for OI data'} tone="var(--accent-yellow)" /><Metric label="BREAKEVEN" value={preview ? preview.metrics.breakevens.map(price).join(' / ') : 'Waiting for OI data'} tone="var(--accent-blue)" /></div>
