...
export default function OiStrategiesPage() {
  ...
  const [wingWidth, setWingWidth] = useState(200);
  ...
  const buildPreview = async () => {
    if (!market) return;
    try { setBusy(true); setError(''); const result = await previewOiStrategy({ strategy, symbol: market.symbol, lots: Number(lots), wing_width: Number(wingWidth) }); setPreview(result.data); }
  ...
  useEffect(() => { buildPreview(); }, [strategy, lots, wingWidth, market]);
  ...
  return <section className="section" style={{ marginTop: 4 }}>
    ...
    <div className="grid grid-4" style={{ marginTop: 14 }}>
      ...
      <Metric label="OI RESISTANCE" value={price(market?.resistance)} tone="var(--accent-red)" />
    </div>
    ...
    {preview && <><div className="grid grid-4" style={{ marginTop: 14 }}><Metric label="MAX PROFIT / LOT" value={money(preview.metrics.max_profit_per_lot)} tone="var(--accent-green)" /><Metric label="MAX LOSS / LOT" value={money(preview.metrics.max_loss_per_lot)} tone="var(--accent-red)" /><Metric label="MARGIN / LOT (EST.)" value={money(preview.metrics.margin_required_per_lot)} tone="var(--accent-yellow)" /><Metric label="BREAKEVEN" value={preview.metrics.breakevens.map(price).join(' / ')} tone="var(--accent-blue)" /></div><div className="card" style={{ marginTop: 14 }}><div className="card-title">Order preview</div><div style={{ fontSize: 12, color: 'var(--text-secondary)', margin: '6px 0 12px' }}>Expiry {preview.expiry} | Lot size {preview.lot_size} | Estimated total margin {money(preview.metrics.margin_required)}</div><table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}><thead><tr><th align="left">Action</th><th align="left">Contract</th><th align="right">Premium</th><th align="right">Quantity</th></tr></thead><tbody>{preview.legs.map((leg) => <tr key={`${leg.side}-${leg.symbol}`}><td style={{ color: leg.side === 'BUY' ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 700 }}>{leg.side}</td><td>{leg.symbol}</td><td align="right">₹{price(leg.premium)}</td><td align="right">{leg.quantity}</td></tr>)}</tbody></table><button className="btn btn-start" style={{ marginTop: 14 }} onClick={placeOrder} disabled={busy || !accountId}>{busy ? 'Processing...' : 'Review and Place Order'}</button>{preview.execution && <div style={{ marginTop: 12, fontSize: 12 }}>{preview.execution.results.map((item) => <div key={item.order_id || item.symbol}>{item.status}: {item.side} {item.symbol} {item.message}</div>)}</div>}</div></>}
  </section>;
}
